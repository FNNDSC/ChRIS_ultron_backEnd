...
from .models import Note, Tag, Feed, Comment, FeedFile, UserFile
...
class FeedFileSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = FeedFile
        fields = ('url', 'path', 'feed_id', 'plugin_inst_id', 'file_resource', 'feed',
                  'plugin_inst')
...
class UserFileSerializer(serializers.HyperlinkedModelSerializer):
    ...
    class Meta:
        model = UserFile
        fields = ('url', 'path', 'fname', 'file_resource', 'owner')
    ...
    def validate_user_path(self, user, path):
        """
        ...
        try:
            ...
            UserFile.objects.get(owner=user.id, path=path)

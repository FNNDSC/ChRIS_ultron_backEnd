...
class PACSFileFilter(FilterSet):
    ...
    max_PatientAge = django_filters.NumberFilter(field_name='PatientAge',
                                                 lookup_expr='lte')
    pacs_identifier = django_filters.CharFilter(method='filter_by_pacs_identifier')
    ...
    def filter_by_pacs_identifier(self, queryset, name, value):
        """
        Custom method to return the files associated to a specific PACS identifier.
        """
        try:
            pacs = PACS.objects.get(identifier=value)
        except PACS.DoesNotExist:
            return PACSFile.objects.none()
        return queryset.filter(fname__startswith=pacs.folder.path)

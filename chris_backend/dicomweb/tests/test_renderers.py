"""
Unit tests for dicomweb.renderers: DICOM JSON Model encoding (PS3.18 §F) and
the DRF renderer wiring for dicomweb views.
"""
import json
...
from dicomweb.dicomnative import DicomAttribute, dataset
...
class DicomJsonEncoderTest(SimpleTestCase):
    ...
    def test_dicom_attribute_not_serializable(self):
        # The encoder does not render attributes: _render_dataset is the
        # single owner of the JSON Model rules, so an attribute reaching the
        # encoder directly fails instead of being emitted without them
        # (e.g. IS '42' as a string).
        with self.assertRaises(TypeError):
            self._encode(DicomAttribute('00200013', 'IS', value=['42']))
...
class DicomJsonRendererTest(SimpleTestCase):
    ...
    def test_bulk_data_rejected_for_any_vr(self):
        # BulkDataURI / InlineBinary are not emitted until WADO-RS (§F.2.6,
        # §F.2.7): an attribute carrying either raises, even on a string VR,
        # rather than silently losing its value.
        with self.assertRaises(ValueError):
            self._render([DicomAttribute('00081196', 'UT', bulk_data='http://x/bulk')])
        with self.assertRaises(ValueError):
            self._render([DicomAttribute('00081196', 'UT', inline_binary=b'\x00\x01')])
    ...
    def test_date_in_dt_keeps_day_precision(self):
        # A date supplied for a DT attribute renders as YYYYMMDD. PS3.5 §6.2:
        # "Trailing null components of Date Time indicate that the Value is
        # not precise to the precision of those components" — a valid DT
        # precise to the day (as pydicom emits it). Widening to midnight
        # would claim a precision the value does not have.
        self.assertEqual(
    ...
    def test_attribute_in_unwalked_container_fails_loudly(self):
        # Only lists, tuples and dicts are walked. An attribute inside any
        # other iterable reaches the encoder, which refuses it rather than
        # emitting it without the JSON Model rules (an IS '42' as a string,
        # a binary VR as InlineBinary).
        attr = DicomAttribute('00200013', 'IS', value=['42'])
        with self.assertRaises(TypeError):
            self._render({'results': (a for a in [attr])})
...
class DicomWebRendererMixinTest(SimpleTestCase):
    ...
    application/json (via Accept, ?format=json or a .json format-suffix
    route) is served by the alias (§10.6.2).
    """
    ...
    def test_format_query_param_selects_json_alias(self):
        # ?format=json is DRF's URL_FORMAT_OVERRIDE (on by default); it
        # selects the alias, never the built-in JSONRenderer.
        response = self._get(path='/?format=json')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.accepted_media_type, 'application/json')
        self.assertEqual(json.loads(response.rendered_content.decode()),
                        self.expected_body)
